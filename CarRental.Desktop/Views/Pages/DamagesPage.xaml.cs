...
public partial class DamagesPage : UserControl
{
    private readonly List<GuideStep> _guideSteps = [];
    private DamagesPageViewModel? _viewModel;
    private int _guideStepIndex = -1;

    public DamagesPage()
    {
        InitializeComponent();
        Loaded += DamagesPage_OnLoaded;
        Unloaded += DamagesPage_OnUnloaded;
        DataContextChanged += DamagesPage_OnDataContextChanged;
    }

    private void DamagesPage_OnLoaded(object sender, RoutedEventArgs e)
    {
        AttachViewModel();
        InitializeGuideSteps();
    }

    private void DamagesPage_OnUnloaded(object sender, RoutedEventArgs e)
    {
    ...
    private void DamagesPage_OnDataContextChanged(object sender, DependencyPropertyChangedEventArgs e)
    {
        AttachViewModel();
        InitializeGuideSteps();
    }
    ...
    private void AttachViewModel()
    {
        ...
        DetachViewModel();
        _viewModel = DataContext as DamagesPageViewModel;
    ...
    {
        if (e.PropertyName == nameof(DamagesPageViewModel.GuideRequestId))
        {
    ...
    private void InitializeGuideSteps()
    {
        ...
        _guideSteps.Add(new GuideStep(
            ...
            "У верхньому блоці вносяться дані про нове пошкодження авто.",
            () => DamageFormGroup));
        _guideSteps.Add(new GuideStep(
            "Додавання запису",
            "Кнопка створює акт пошкодження за заповненими полями.",
            () => AddDamageButton));
        _guideSteps.Add(new GuideStep(
            "Історія пошкоджень",
            "Тут відображаються всі зафіксовані пошкодження, статус і суми.",
            () => DamageHistoryGroup));
        _guideSteps.Add(new GuideStep(
            "Оновлення даних",
            "Кнопка внизу оновлює список пошкоджень з бази.",
            () => RefreshDamagesButton));
        _guideSteps.Add(new GuideStep(
            "Панель повідомлень",
            "Тут показується результат операцій: успіх або помилка.",
            () => DamagesStatusPanel));
    }
    ...
    {
        var rootWidth = Math.Max(DamagesRoot.ActualWidth, 1);
        var rootHeight = Math.Max(DamagesRoot.ActualHeight, 1);

        var bounds = target.TransformToAncestor(DamagesRoot).TransformBounds(new Rect(0, 0, target.ActualWidth, target.ActualHeight));
        bounds.Inflate(8, 8);
