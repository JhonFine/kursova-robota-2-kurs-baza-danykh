...
public partial class MaintenancePage : UserControl
{
    ...
    private MaintenancePageViewModel? _viewModel;
    private int _guideStepIndex = -1;

    public MaintenancePage()
    {
        InitializeComponent();
        Loaded += MaintenancePage_OnLoaded;
        Unloaded += MaintenancePage_OnUnloaded;
        DataContextChanged += MaintenancePage_OnDataContextChanged;
    }

    private void MaintenancePage_OnLoaded(object sender, RoutedEventArgs e)
    {
        AttachViewModel();
        InitializeGuideSteps();
    }

    private void MaintenancePage_OnUnloaded(object sender, RoutedEventArgs e)
    {
    ...
    private void MaintenancePage_OnDataContextChanged(object sender, DependencyPropertyChangedEventArgs e)
    {
        AttachViewModel();
        InitializeGuideSteps();
    }
    ...
    private void AttachViewModel()
    {
        ...
        DetachViewModel();
        _viewModel = DataContext as MaintenancePageViewModel;
    ...
    {
        if (e.PropertyName == nameof(MaintenancePageViewModel.GuideRequestId))
        {
    ...
    private void InitializeGuideSteps()
    {
        ...
        _guideSteps.Add(new GuideStep(
            "Запис про техобслуговування",
            "Тут обирається авто і вводяться параметри нового запису ТО.",
            () => MaintenanceRecordGroup));
        _guideSteps.Add(new GuideStep(
            "Додавання запису",
            "Кнопка створює новий запис ТО у базі за введеними даними.",
            () => AddRecordButton));
        _guideSteps.Add(new GuideStep(
            "Історія ТО",
            "Ця вкладка показує журнал усіх виконаних техобслуговувань.",
            () => RecordsGrid,
            SelectHistoryTab));
        _guideSteps.Add(new GuideStep(
            "Планове ТО",
            "Тут видно авто, для яких наближається або прострочене обслуговування.",
            () => DueItemsGrid,
            SelectDueTab));
        _guideSteps.Add(new GuideStep(
            "Панель повідомлень",
            "Нижній рядок відображає результат додавання запису або помилку валідації.",
            () => MaintenanceStatusPanel));
    }

    private void SelectHistoryTab()
    {
        MaintenanceTabControl.SelectedItem = HistoryTab;
    }

    private void SelectDueTab()
    {
        MaintenanceTabControl.SelectedItem = DueTab;
    }
    ...
    {
        var rootWidth = Math.Max(MaintenanceRoot.ActualWidth, 1);
        var rootHeight = Math.Max(MaintenanceRoot.ActualHeight, 1);

        var bounds = target.TransformToAncestor(MaintenanceRoot).TransformBounds(new Rect(0, 0, target.ActualWidth, target.ActualHeight));
        bounds.Inflate(8, 8);
