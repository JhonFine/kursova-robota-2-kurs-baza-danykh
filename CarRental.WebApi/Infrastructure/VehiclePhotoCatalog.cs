...
internal static class VehiclePhotoCatalog
{
    ...
    public static bool TryResolveStoredPhotoPath(string? storedPath, out string? fullPath)
    {
        ...
        if (TryResolveStaticPhotoPath(trimmedPath, requireFileExists: true, out _, out fullPath))
        {
            return true;
        }
    ...
    public static bool TryNormalizeStoredPhotoPath(string? sourcePath, bool requireFileExists, out string? normalizedPath)
    {
        normalizedPath = null;
        ...
        if (!TryResolveStaticPhotoPath(sourcePath.Trim(), requireFileExists, out normalizedPath, out _))
        {
            return false;
        }
    ...
    private static bool TryResolveStaticPhotoPath(
        string sourcePath,
        ...
        out string? fullPath)
    {
        ...
        if (relativePath is null)
        {
            ...
            {
                var candidateFullPath = Path.GetFullPath(sourcePath);
                if (!IsPathInsideRoot(candidateFullPath, staticDirectory))
                {
                    return false;
                }

                relativePath = Path.GetRelativePath(staticDirectory, candidateFullPath)
                    .Replace('\\', '/')
                    .TrimStart('/');
            }
            else
        ...
        normalizedPath = $"{StaticImagesRoot}/{cleanedRelativePath.Replace('\\', '/')}";
